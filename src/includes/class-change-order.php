...
add_action('woocommerce_checkout_process', function () {
    $LOG = [
        "type" => "woocommerce_checkout_process",
    ];
    try {

        // Método de pago seleccionado
        $payment_method = isset($_POST['payment_method'])
            ? wc_clean($_POST['payment_method'])
            : '';
        ...
        /**
         * Detectar si el envío seleccionado
         * es contraentrega
         */
        $is_shipping_contraentrega = false;
        ...
        foreach ($shipping_methods as $method) {
             ...
             * Ejemplo IDs:
             *
             * transportadora_wc_contraentrega_on
             * transportadora_wc_contraentrega_off
             */

            if (strpos($method, 'wc_contraentrega_on') !== false) {
                $is_shipping_contraentrega = true;
                break;
            }
        }

        /**
         * Detectar si el pago es contraentrega
         */
        $is_payment_contraentrega =
            $payment_method === AVSHME_PAYMENT_CONTRAENTREGA;

        /**
         * VALIDACIONES
         */

        // Pago contraentrega PERO envío normal
        if (
            $is_payment_contraentrega &&
            !$is_shipping_contraentrega
        ) {

            wc_add_notice(
                __('Debes seleccionar un envío contraentrega para usar pago contraentrega.', 'woocommerce'),
                'error'
            );
        ...
        // Pago normal PERO envío contraentrega
        if (
            !$is_payment_contraentrega &&
            $is_shipping_contraentrega
        ) {
            ...
            wc_add_notice(
                __('No puedes seleccionar un envío contraentrega con un método de pago normal.', 'woocommerce'),
                'error'
            );
